...
/**
 * Admin edit screen for one video. The static /new route takes precedence
 * in Next routing, so this file only ever sees numeric ids.
 */
export default function AdminVideoEditPage() {
  ...
  if (hasToken === false) {
    return (
      <>
        ...
        />
        <Link
          href={`/admin/login?next=/admin/videos/${raw}`}
  ...
  if (!isValidId) {
    return (
      <AdminPageHeader
        ...
        description={`"${raw}" is not a valid video id. Open one from the list.`}
        actions={
          <Link
            href="/admin/videos"
  ...
  return <VideoEditor mode="edit" id={idNum} />;
}
